...
- [`board-bringup-notes.md`](board-bringup-notes.md) — hardware findings from bringing up new boards that would otherwise bite the next one too (not architecture, a running lessons list).
- [`package-signing.md`](package-signing.md) — why HMAC only answers "may this install here," not "who built this" — the pack/push signing split, the forward-compatible `sig` field format, and the (documented, not built) asymmetric provenance concept.
